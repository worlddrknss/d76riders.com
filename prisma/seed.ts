// Development seed. Creates demo riders, events, roads, bikes, and activity from
// src/data/community.ts — DO NOT run this against production.
//
// For production, use `npm run db:seed:catalog`, which seeds only the reference
// data the reputation and community features read (badges, skill tracks, quests,
// crews) and creates no content.
...
async function main() {
  ...
  // Badge/skill/quest/crew definitions live in seed-catalog.ts, which is also
  // the prod-safe entrypoint (npm run db:seed:catalog).
