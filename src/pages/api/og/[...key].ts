 ...
 * every response, and Facebook's image scraper rejects the (valid) JPEG because
 * of it — the plain static site image, which has no such header, scrapes fine.
 * A Pages API route returns a clean image response with no RSC Vary.
 *
 * Uploads are stored as WebP (unrenderable by most scrapers), so we transcode to
 * a baseline JPEG on the fly. The public URL is `<name>.jpg` (stored extension
 * stripped) so no ".webp" appears anywhere for extension-sniffing scrapers.
 */
