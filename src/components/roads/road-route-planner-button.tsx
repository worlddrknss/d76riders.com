...
import { saveRoadRouteAction } from "@/app/(site)/roads/actions";
...
/**
 * The single entry point for road route planning, sitting with the other
 * road tools. Drawing and saving here attaches the route to a road that
 * never had one, or replaces the existing route — so route planning doesn't have
 * to happen while the event is being created.
 */
export function RoadRoutePlannerButton({
  roadId,
  hasRoute,
}: {
  roadId: string;
  ...
  function save() {
    ...
    startTransition(async () => {
      const result = await saveRoadRouteAction(roadId, {
          ...
          <div className="safe-pb fixed inset-0 z-[9999] flex flex-col bg-canvas">
            <div className="safe-pt flex items-center justify-between gap-3 border-b border-border bg-surface px-4 py-3 sm:px-6">
              <div className="min-w-0">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-sunset">Road Route Builder</p>
                <h3 className="font-display text-xl text-asphalt">
                  {hasRoute ? "Replace the official route" : "Official Road Route"}
                </h3>
                <p className="mt-1 text-xs text-muted">
                  {error ? (
                    ...
                    "Draw or trace a route, then save it to attach it to this road."
                  )}
                </p>
              </div>
