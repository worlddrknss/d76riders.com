...
/**
 * OpenGraph/social image endpoint. Uploads are stored as WebP, which many link
 * scrapers can't render, so this transcodes to a baseline JPEG. The public URL is
 * a clean `<name>.jpg` (stored extension stripped) so no ".webp" appears anywhere.
 *
 * Note: Facebook itself rejects this (valid) JPEG for reasons we couldn't
 * reproduce from our side — it's not the bytes, format, colour space, headers, or
 * Vary. Other scrapers (Slack/Discord/iMessage/Twitter) render it fine, so the
 * route stays; Facebook falls back to the site's static default image.
 */
